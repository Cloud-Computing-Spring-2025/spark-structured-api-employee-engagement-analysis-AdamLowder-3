...
def map_engagement_level(df):
    ...
    # 'Medium' -> 2
    # 'High' -> 3
    return df.withColumn("EngagementScore", when(col("EngagementLevel") == "Low", 1).when(col("EngagementLevel") == "Medium", 2).when(col("EngagementLevel") == "High", 3))
...
def compare_engagement_levels(df):
    ...
    return df.groupBy("JobTitle").agg(spark_round(avg("EngagementScore"), 2).alias("AvgEngagementLevel")).orderBy(col("AvgEngagementLevel").desc())  # Sort by highest engagement
...
def main():
    ...
    output_file = "/workspaces/spark-structured-api-employee-engagement-analysis-AdamLowder-3/outputs/task3/engagement_levels_job_titles.csv"
